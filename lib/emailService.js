...
export const sendTeacherInvitationEmail = async (email, teacherName, invitationToken, inviterName) => {
    ...
    if (isBrevoTeacherResend && hasBrevoApiTeacher) {
        ...
        try {
            ...
            const sendSmtpEmail = new brevo.SendSmtpEmail();
            sendSmtpEmail.subject = `Invitation to Join Bainum Project as a Teacher`;
...
/**
 * Send coach invitation email. Same provider chain as the teacher
 * invitation (Brevo API → Resend → SMTP) in a compact form.
 */
export const sendCoachInvitationEmail = async (email, coachName, invitationToken, inviterName) => {
    const isDeployProduction = process.env.NODE_ENV === 'production' || !!process.env.RENDER;
    const useProductionInviteLink =
        isDeployProduction ||
        !!(process.env.FRONTEND_URL && !process.env.FRONTEND_URL.includes('localhost'));
    ...
    baseUrl = baseUrl.replace(/\/$/, '');
    const invitationLink = `${baseUrl}/coach/register?token=${invitationToken}`;

    const subject = 'Invitation to Join Bainum Project as a Coach';
    const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    <p>Hello ${coachName},</p>
                    <p>You have been invited by <strong>${inviterName}</strong> to join the Bainum Project as a coach.</p>
                    <p>Click the button below to create your account and get started:</p>
                    <div style="text-align: center;">
                        <a href="${invitationLink}" class="button">Accept Invitation</a>
                    </div>
    ...
    const textContent = `
        ...
        You have been invited by ${inviterName} to join the Bainum Project as a coach.
    ...
    if (!isBrevoConfigured && resend) {
        ...
        const freeEmailDomains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'aol.com'];
        if (freeEmailDomains.includes(fromEmail.split('@')[1]?.toLowerCase())) {
            fromEmail = 'onboarding@resend.dev';
        }
        const fromName = process.env.EMAIL_FROM_NAME || 'Bainum Project';
        const data = await resend.emails.send({
    ...
    if (isProduction && !isBrevoConfigured && !resend) {
