...
// Initialize Brevo API if API key is provided
let brevoApi = null;
if (process.env.BREVO_API_KEY?.trim()) {
    try {
        brevoApi = new brevo.TransactionalEmailsApi();
        brevoApi.setApiKey(brevo.TransactionalEmailsApiApiKeys.apiKey, process.env.BREVO_API_KEY.trim());
        console.log('Brevo API initialized successfully');
    } catch (error) {
        console.error('Failed to initialize Brevo API:', error);
    }
} else {
    console.log('Brevo API key not found, will use SMTP if configured');
}
...
function buildParentInvitationPayload(
    ...
    const middleHtml = usePartner
        ? `<p>If you are a parent, please try our Enact application for child language development at your home.</p>
                            ...
                            <p>When you have completed that step, use the link below to finish registering for the CATTAC parent portal and access your ${childPossessive} data:</p>
                            <div style="text-align: center;">
                                <a href="${invitationLink}" class="button">Complete CATTAC Registration</a>
                            </div>
                            <p>CATTAC registration link (copy and paste if needed):</p>
...
When you have completed that step, finish your CATTAC parent portal registration with this link:
...
${invitationLink}`;
    ...
    const htmlContent = `
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            <p>Hello,</p>
                            <p>You have been invited by <strong>${inviterName}</strong> to join the CATTAC parent portal for <strong>${childName}</strong>.</p>
                        ...
                        <div class="footer">
                            <p>This is an automated message from the CATTAC system.</p>
                        </div>
                    </div>
                </body>
                </html>
    ...
    const textContent = `
                ...
                You have been invited by ${inviterName} to join the CATTAC parent portal for ${childName}.
                ...
                This is an automated message from the CATTAC system.
            `;
...
export const sendInvitationEmail = async (email, childName, invitationToken, inviterName, options = {}) => {
    ...
    if (isBrevoForResend && hasBrevoApi) {
        ...
        try {
            ...
            const fromName = process.env.EMAIL_FROM_NAME || 'CATTAC';
...
export const sendTeacherInvitationEmail = async (email, teacherName, invitationToken, inviterName) => {
    ...
    const hasBrevoApiTeacher = !!brevoApi;
    
    // If Brevo API is available, use it (preferred over SMTP)
    if (isBrevoTeacherResend && hasBrevoApiTeacher) {
        console.log('Using Brevo API to send teacher invitation email');
        try {
            const fromEmail = process.env.EMAIL_FROM_EMAIL?.trim() || process.env.EMAIL_USER?.trim() || 'noreply@bainumproject.com';
            const fromName = process.env.EMAIL_FROM_NAME || 'CATTAC';
            
            const sendSmtpEmail = new brevo.SendSmtpEmail();
            sendSmtpEmail.subject = `Invitation to Join CATTAC as a Teacher`;
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            ...
                            <p>You have been invited by <strong>${inviterName}</strong> to join CATTAC as a teacher.</p>
            ...
            sendSmtpEmail.textContent = `
                ...
                You have been invited by ${inviterName} to join CATTAC as a teacher.
            ...
            const data = await brevoApi.sendTransacEmail(sendSmtpEmail);
            console.log('Teacher invitation email sent successfully via Brevo API:', {
                to: email,
                messageId: data.messageId
            });
            ...
            console.error('Brevo API error details for teacher invitation:', {
                message: error.message,
                response: error.response?.body,
                status: error.status
            });
            throw new Error(`Failed to send teacher invitation email via Brevo API: ${error.message || 'Unknown error'}`);
        }
    }
    
    // If Brevo is explicitly configured, skip Resend entirely
    if (isBrevoTeacherResend) {
        console.log('Brevo is configured for teacher invitation, skipping Resend API');
    } else if (resend) {
        try {
            const htmlContent = `
                <!DOCTYPE html>
            ...
            const data = await resend.emails.send({
                from: `${fromName} <${fromEmail}>`,
                to: [email],
                subject: `Invitation to Join CATTAC as a Teacher`,
                html: htmlContent,
                text: textContent,
            });
    ...
    try {
        ...
        const fromNameTeacher = process.env.EMAIL_FROM_NAME || 'CATTAC';
...
export const sendPasswordResetEmail = async (email, resetToken) => {
    ...
    if (isBrevoForResend && hasBrevoApi) {
        try {
            ...
            const sendSmtpEmail = new brevo.SendSmtpEmail();
            sendSmtpEmail.subject = `Reset Your Password - CATTAC`;
            sendSmtpEmail.htmlContent = `
                <!DOCTYPE html>
                <html>
                <head>
                ...
                <body>
                    <div class="container">
                        ...
                        <div class="content">
                            <p>You requested a password reset for your CATTAC account.</p>
                        ...
                        <div class="footer"><p>This is an automated message from the CATTAC system.</p></div>
                    </div>
                </body>
                </html>
            `;
    ...
    if (!isBrevoForResend && resend) {
        try {
            ...
            await resend.emails.send({
                from: `${fromName} <${fromEmail}>`,
                to: [email],
                subject: `Reset Your Password - CATTAC`,
                html: htmlContent,
            });
            return { success: true };
        } catch (error) {
