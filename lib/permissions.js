...
export const CAPABILITIES = Object.freeze({
    ...
    /** Classroom recording flow: teachers (own classrooms) and coaches (active grant). */
    ...
    /** Grant/revoke the transcript tier on a home-view grant: admins only. */
    grantHomeTranscriptAccess: ["admin"],
    /** View the teacher/coach activity log in Settings: admins only. */
...
export async function canViewClassroomAggregates(user, classroom) {
    ...
    if (user.role === "parent" && isEnrolledParent(user, classroom)) {
        const { resolveParentClassroomAccess } = await import("./viewerAccessService.js");
        const access = await resolveParentClassroomAccess(idOf(user), classroom);
        return access.allowed;
    }
    if (user.role === "coach") {
        const { resolveCoachClassroomAccess } = await import("./viewerAccessService.js");
        const access = await resolveCoachClassroomAccess(idOf(user), classroom);
        return access.allowed;
    }
    return false;
}
...
/**
 * Who may upload or accept a classroom recording: teachers who manage the
 * classroom, or coaches holding an ACTIVE grant. Admins and parents are
 * denied. This does not widen saved-transcript reads.
 */
export async function canRecordInClassroom(user, classroom) {
    if (!user || !classroom) return false;
    if (user.role === "teacher" && canManageClassroom(user, classroom)) return true;
    if (user.role === "coach") {
        const { resolveCoachClassroomAccess } = await import("./viewerAccessService.js");
        const access = await resolveCoachClassroomAccess(idOf(user), classroom);
        return access.allowed;
...
/**
 * TeacherAssessment.teacherId for a classroom recording. Teachers keep their
 * own id. Coaches are attributed to the classroom lead (assistant if the
 * lead slot is empty). Returns null when neither slot is set.
 */
export function resolveClassroomRecordingTeacherId(user, classroom) {
...
export async function canViewClassroomTranscripts(user, classroom) {
    ...
    if (user.role === "coach") {
        const { resolveCoachClassroomAccess } = await import("./viewerAccessService.js");
        const access = await resolveCoachClassroomAccess(idOf(user), classroom);
        return access.allowed && access.transcript;
    }
    return false;
}
...
export async function coachClassroomTier(coachId, classroomId) {
    ...
    const { resolveCoachClassroomAccess } = await import("./viewerAccessService.js");
    const access = await resolveCoachClassroomAccess(coachId, classroom);
    if (!access.allowed) return "none";
    return access.transcript ? "transcripts" : "aggregate";
}
