...
async function createHomeTranscriptAccessChangedNotification({
    recipientId,
    recipientRole,
    child,
    message,
}) {
    if (!recipientId || !recipientRole || !child || !message) return null;
    try {
        return await Notification.create({
            recipientId,
            recipientRole,
            type: "home-transcript-access-changed",
            childId: child._id ?? child.id ?? null,
            childName: childNameOf(child),
            message,
            expiresAt: buildExpiresAt(),
        });
    } catch (error) {
        console.error(
            "[notificationService] createHomeTranscriptAccessChangedNotification failed:",
            error.message
        );
        return null;
    }
...
/**
 * Notify the child's linked parents — and the user-scoped grantee, when
 * there is one — that an admin changed the transcript tier on a home-view
 * grant. Callers MUST NOT roll back the toggle when this fails.
 */
export async function fanOutHomeTranscriptAccessChangedNotifications({
    child,
    parentIds,
    grantee,
    granteeName,
    ...
    const name = childNameOf(child);
    const created = [];
    const parentMessage = transcriptAccess
        ? `An admin enabled home talk transcript access for ${granteeName} on ${name}'s data`
        : `An admin removed home talk transcript access for ${granteeName} on ${name}'s data`;
    for (const parentId of parentIds || []) {
        if (!parentId) continue;
        const doc = await createHomeTranscriptAccessChangedNotification({
            recipientId: parentId,
            recipientRole: "parent",
            child,
    ...
    if (grantee?.id && grantee?.role) {
        const doc = await createHomeTranscriptAccessChangedNotification({
            recipientId: grantee.id,
            recipientRole: grantee.role,
            child,
            message: transcriptAccess
                ? `You now have transcript access to ${name}'s home talk data`
                : `Your transcript access to ${name}'s home talk data was removed`,
        });
        if (doc) created.push(doc);
    }
    return created;
...
/**
 * Coach grant lifecycle notifications. One creator handles all five
 * coach-related types; callers pass the type and a ready message.
 * Same failure mode as the other creators (never throws, returns null).
 */
export async function createCoachGrantNotification({
    recipientId,
    recipientRole,
    type,
    classroom,
    ...
    if (!recipientId || !recipientRole || !type || !message) return null;
    try {
        return await Notification.create({
            recipientId,
            recipientRole,
            type,
            classroomId: classroom?._id ?? classroom?.id ?? null,
            classroomName: classroomNameOf(classroom),
            message,
            expiresAt: buildExpiresAt(),
        });
    } catch (error) {
        console.error(
            "[notificationService] createCoachGrantNotification failed:",
            error.message
        );
        return null;
    }
...
export async function createChartAccessNotification({
    recipientId,
    recipientRole,
    place,
    classroom,
    ...
    const onOff = chartsOn ? "on" : "off";
    const label =
        place === "home"
            ? childNameOf(child) || "home talk"
            : classroomNameOf(classroom) || "a classroom";
    const message =
        place === "home"
            ? `Home charts for ${label} were turned ${onOff}`
            : `Classroom charts for "${label}" were turned ${onOff}`;
    try {
        return await Notification.create({
            recipientId,
            recipientRole,
            type: "chart-access-changed",
            classroomId: classroom?._id ?? classroom?.id ?? null,
            classroomName: classroomNameOf(classroom),
            childId: child?._id ?? child?.id ?? null,
            message,
        ...
        console.error("[notificationService] createChartAccessNotification failed:", error.message);
        return null;
    }
}
