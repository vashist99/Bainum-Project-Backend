...
import {
    findPersonSpans,
    findPersonSpansCompromise,
} from "./pii/personRecognizer.js";
...
function redactWithPersonSpans(text, personSpans) {
...
export async function redactPii(text) {
    ...
    try {
        return redactWithPersonSpans(text, await findPersonSpans(text));
    } catch (err) {
        console.error("[pii] primary recognizer failed; using pattern+compromise:", err?.name || "Error");
        try {
            return redactWithPersonSpans(text, findPersonSpansCompromise(text));
        } catch {
            return redactWithPersonSpans(text, []);
        }
    }
}
