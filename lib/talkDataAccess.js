...
export function isStaffRole(role) {
    return role === "teacher" || role === "admin" || role === "coach";
}
...
export async function staffHasHomeViewAccess(user, childId) {
    ...
    const { canViewerSeeHomeCharts } = await import("./viewerAccessService.js");
    return canViewerSeeHomeCharts(user, childId);
}
...
export async function staffHasHomeTranscriptAccess(user, childId) {
    if (!user?.id || !childId || !isStaffRole(user.role)) return false;
    const { canViewerSeeHomeTranscripts } = await import("./viewerAccessService.js");
    return canViewerSeeHomeTranscripts(user, childId);
}
