...
/** Closed list stored on Classroom.ageGroup. Empty means unset. */
export const CLASSROOM_AGE_GROUPS = Object.freeze([
    "Infant",
    "Toddler",
    "Preschool",
    "Pre-K",
...
/**
 * Normalize an age-group input. Missing or blank becomes unset.
 * Any other value is rejected.
 */
export function parseAgeGroup(value) {
    if (value == null || value === "") return { ok: true, ageGroup: null };
    if (typeof value === "string" && CLASSROOM_AGE_GROUPS.includes(value)) {
        return { ok: true, ageGroup: value };
    }
    return {
        ok: false,
        message: "Age group must be Infant, Toddler, Preschool, Pre-K, or Mixed ages",
    };
}
