...
const ORIGIN_BANNER =
    "This bug report was submitted by a signed-in user of the CATTAC dashboard and sent to this address at their request.";
...
export function buildBugReportEmailPayload(report) {
    ...
    const subject = `[Bug Report] ${title} — CATTAC Dashboard`;
...
${htmlSection("Description", description)}${stepsToReproduce ? htmlSection("Steps to reproduce", stepsToReproduce) : ""}${pageOrFeature ? htmlSection("Where it happened", pageOrFeature) : ""}
    ...
    textLines.push("", "This is an automated message from the CATTAC system.");
