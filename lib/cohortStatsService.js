...
import { homeOnlyContextFilter } from "./talkDataAccess.js";
...
export async function recomputeAndSaveChildrenCohortStats() {
    // Home-talk baseline: the child data page renders home recordings only,
    // so its dial thresholds are computed from home-context assessments.
    const assessments = await Assessment.find({
        ...homeOnlyContextFilter(),
        hidden: { $ne: true },
    }).select("childId categoryWPM");
...
export async function recomputeAndSaveTeachersCohortStats() {
    const assessments = await TeacherAssessment.find({ hidden: { $ne: true } }).select("teacherId categoryWPM");
